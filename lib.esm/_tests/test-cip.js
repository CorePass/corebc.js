...
import { Interface, Cip150MetadataContract, Cip151Lifecycle, Cip152LabCertificate, CustomUnitToken, cip150Abi, customUnitAbi, IpfsGateway, } from "../index.js";
...
describe("IPFS gateway", () => {
    it("resolves references and rejects unsupported schemes and traversal", () => {
        ...
        assert.throws(() => new IpfsGateway({ template: "ftp://example.com/{cid}" }));
    });
    ...
    it("rejects HTTP errors and invalid JSON", async () => {
        await assert.rejects(new IpfsGateway({
            fetch: async () => new Response("no", { status: 404 }),
        }).readJson("QmExample"), /404/);
        await assert.rejects(new IpfsGateway({ fetch: async () => new Response("not json") }).readJson("QmExample"), SyntaxError);
    });
