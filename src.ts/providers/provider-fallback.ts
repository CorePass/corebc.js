...
function getMedian(
  quorum: number,
  results: Array<TallyResult>,
): undefined | bigint | Error {
  let resultWeight = 0;
...
function getFuzzyMode(
  quorum: number,
  results: Array<TallyResult>,
): undefined | number {
  if (quorum === 1) {
    return getNumber(<bigint>getMedian(quorum, results), "%internal");
  }
...
export class FallbackProvider extends AbstractProvider {
  ...
  async _detectNetwork(): Promise<Network> {
    return Network.from(
      getBigInt(await this._perform({ method: "networkId" })),
    );
  }
