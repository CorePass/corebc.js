...
export class AbstractProvider implements Provider {
  ...
  async getNetwork(): Promise<Network> {
    ...
    if (expected.networkId !== actual.networkId) {
      if (this.#anyNetwork) {
        ...
        assert(
          false,
          `network changed: ${expected.networkId} => ${actual.networkId} `,
          "NETWORK_ERROR",
          {
            event: "changed",
          },
  ...
  async getEnergyPrice(): Promise<FeeData> {
  ...
  async #call(
    ...
    try {
      return hexlify(
        await this._perform({ method: "call", transaction, blockTag }),
      );
    } catch (error) {
  ...
  async #getAccountValue(
    request: _PerformAccountRequest,
    ...
    return await this.#checkNetwork(
      this.#perform(Object.assign(request, { address, blockTag })),
    );
  }
  ...
  async getTransactionReceipt(
    ...
    if (params.energyPrice == null && params.effectiveEnergyPrice == null) {
      ...
      if (tx == null) {
        throw new Error(
          "report this; could not find tx or effectiveEnergyPrice",
        );
      }
