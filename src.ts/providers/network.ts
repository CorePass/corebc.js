...
import { EnergyCostPlugin } from "./plugins-network.js";
...
export class Network {
	...
	computeIntrinsicEnergy(tx: TransactionLike): number {
		const costs =
			this.getPlugin<EnergyCostPlugin>(
				"org.corebc.plugins.network.EnergyCost",
			) || new EnergyCostPlugin();
		...
		if (typeof network === "object") {
			assertArgument(
				typeof network.name === "string" &&
					typeof network.networkId === "number",
				"invalid network object name or networkId",
				"network",
				network,
			);
	 ...
	 */
	static register(
		nameOrnetworkId: string | number | bigint,
		networkFunc: () => Network,
	): void {
