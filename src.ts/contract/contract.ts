...
function getRunner<T extends ContractRunner>(
	value: any,
	feature: keyof ContractRunner,
): null | T {
	if (value == null) {
		return null;
	}
...
function buildWrappedFallback(contract: BaseContract): WrappedFallback {
	const populateTransaction = async function (
		...
		const tx: ContractTransaction = <any>(
			await copyOverrides<"data">(overrides, ["data"])
		);
	...
	const estimateEnergy = async function (
		overrides?: Omit<TransactionRequest, "to">,
	): Promise<bigint> {
		const runner = getRunner(contract.runner, "estimateEnergy");
		assert(
			canEstimate(runner),
			"contract runner does not support energy estimation",
			"UNSUPPORTED_OPERATION",
...
>(contract: BaseContract, key: string): BaseContractMethod<A, R, D> {
	const getFragment = function (
		...
		assert(
			fragment,
			"no matching fragment in getFragment",
			"UNSUPPORTED_OPERATION",
			{
				operation: "fragment",
			},
	...
	Object.defineProperty(method, "fragment", {
		...
		get: () => {
			...
			assert(
				fragment,
				"no matching fragment in defineProperty",
				"UNSUPPORTED_OPERATION",
				{
					operation: "fragment",
				},
...
function buildWrappedEvent<A extends Array<any> = Array<any>>(
	contract: BaseContract,
	key: string,
): ContractEvent<A> {
