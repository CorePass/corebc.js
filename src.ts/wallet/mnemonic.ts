...
function mnemonicToEntropy(
  ...
  const checksum =
    getBytes(legacySha256(entropy.slice(0, entropyBits / 8)))[0] & checksumMask;
...
function entropyToMnemonic(
  ...
  const checksum =
    parseInt(legacySha256(entropy).substring(2, 4), 16) &
  ...
  return wordlist.join(
    indices.map((index) => (<Wordlist>wordlist).getWord(index)),
  );
}
