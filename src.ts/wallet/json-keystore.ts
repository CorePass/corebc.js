...
import { computeAddress, extractPrefix } from "../transaction/address.js";
...
function getAccount(data: any, _key: string): KeystoreAccount {
	...
	const computedMAC = hexlify(
		sha256(concat([key.slice(16, 32), ciphertext])),
	).substring(2);
	const expectedMAC = spelunk<string>(data, "crypto.mac:string!");
	let difference = 0;
	...
	if (version === "0.1") {
		...
		const mnemonicCiphertext = spelunk<Uint8Array>(
			data,
			"x-corebc.mnemonicCiphertext:data!",
		);
		const mnemonicIv = spelunk<Uint8Array>(
			data,
			"x-corebc.mnemonicCounter:data!",
		);

		const mnemonicAesCtr = new CTR(mnemonicKey, new Counter(mnemonicIv));

		account.mnemonic = {
			path: spelunk<null | string>(data, "x-corebc.path:string") || defaultPath,
			locale: spelunk<null | string>(data, "x-corebc.locale:string") || "en",
...
function getDecryptKdfParams<T>(data: any): KdfParams {
	...
	if (kdf && typeof kdf === "string") {
		if (kdf.toLowerCase() === "scrypt") {
			...
			assertArgument(
				prf === "hmac-sha256" || prf === "hmac-sha512",
				"invalid kdf.pdf",
				"kdf.pdf",
				prf,
			);
 ...
 */
export function decryptKeystoreJsonSync(
	json: string,
...
function _encryptKeystore(
	...
	if (account.mnemonic) {
		const client =
			options.client != null ? options.client : `corebc/${version}`;
