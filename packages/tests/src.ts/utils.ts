...
function randomBytes(seed: string, lower: number, upper?: number): Uint8Array {
    ...
    let result = ethers.utils.arrayify(ethers.utils.sha256(ethers.utils.toUtf8Bytes(seed)));
    while (result.length < upper) {
        result = ethers.utils.concat([result, ethers.utils.sha256(ethers.utils.concat([seed, result]))]);
    }

    let top = ethers.utils.arrayify(ethers.utils.sha256(result));
