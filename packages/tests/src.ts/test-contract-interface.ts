...
function equals(actual: any, expected: any): boolean {
    ...
    // Uint8Array
    if (expected.buffer) {
        if (!corebc.utils.isHexString(actual)) { return false; }
        actual = corebc.utils.arrayify(actual);
    ...
    // Maybe address?
    try {
        let actualAddress = corebc.utils.getAddress(actual);
        let expectedAddress = corebc.utils.getAddress(expected);
...
function getValues(object: any, named?: boolean): any {
   ...
   switch (object.type) {
       ...
       case 'buffer':
           return corebc.utils.arrayify(object.value);
...
describe('ABI Coder Encoding', function() {
    let coder = corebc.utils.defaultAbiCoder;
...
describe('Test Contract Events', function() {
    ...
    tests.forEach((test, index) => {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function() {
            ...
            let iface = new corebc.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function() {
            ...
            test.normalizedValues.forEach((expected, index) => {
                if (test.indexed[index]) {
                    assert.ok((corebc.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                } else {
...
describe('Test Interface Signatures', function() {
    ...
    tests.forEach((test) => {
        it('derives the correct signature - ' + test.name, function() {
            let iface = new corebc.utils.Interface(test.abi);
            this.timeout(120000);
    ...
    it('derives correct description for human-readable ABI', function() {
        let iface = new corebc.utils.Interface([ "function transfer(address from, uint amount)" ]);
        [
            "transfer",
    ...
    it ('parses transaction function', function() {
        ...
        let tx = corebc.utils.parseTransaction(rawTx);
...
describe('Test Number Coder', function() {
    ...
    it('encodes zero', function() {
        let ZeroValues = [
            ...
            { n: 'BigNumber', v: corebc.constants.Zero }
        ];
    ...
    it('encodes one', function() {
        let ZeroValues = [
            ...
            { n: 'BigNumber', v: corebc.constants.One }
        ];
    ...
    it('encodes negative one', function() {
        let Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: corebc.constants.NegativeOne }
        ];
    ...
    it('encodes full uint8 range', function() {
        for (let i = 0; i < 256; i++) {
            ...
            expected += corebc.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function() {
        for (let i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += corebc.utils.hexlify(256 + i).substring(2);
            } else {
...
describe('Test Filters', function() {
    ...
    function doTest(test: TestCase) {
        it(test.name, function() {
            let iface = new corebc.utils.Interface([ test.signature ]);
...
describe("Test ParamType Parser", function() {
    ...
    Tests.forEach((test) => {
        it(`allows correct modifiers ${ JSON.stringify(test.type) }`, function() {
            const paramType = corebc.utils.ParamType.from(test.type);
...
describe('Test EIP-838 Error Codes', function() {
    ...
    it("testError1", async function () {
        ...
        const contract = new corebc.Contract(addr, [
        ...
        try {
            ...
            assert.equal(error.code, corebc.utils.Logger.errors.CALL_EXCEPTION, "error.code");
...
describe("Additional test cases", function() {
    ...
    it("allows addresses without the 0x", function() {
        const iface = new corebc.utils.Interface([
