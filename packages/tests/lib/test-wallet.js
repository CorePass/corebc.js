...
describe('Test JSON Wallets', function () {
    var tests = (0, corebc_testcases_1.loadTests)('wallets');
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            if (test.hasAddress) {
                                assert_1.default.ok((corebc_1.corebc.utils.getJsonWalletAddress(test.json) !== null), 'detect encrypted JSON wallet');
                            }
                            return [4 /*yield*/, corebc_1.corebc.Wallet.fromEncryptedJson(test.json, test.password)];
                        case 1:
                            wallet = _a.sent();
                        ...
                        case 2:
                            ...
                            // Test connect
                            {
                                provider = corebc_1.corebc.getDefaultProvider("");
                            ...
                            {
                                wallet2 = new corebc_1.corebc.Wallet(wallet._signingKey(), wallet.prefix);
                            ...
                            if (test.name === "life") {
                                wallet2 = corebc_1.corebc.Wallet.fromEncryptedJsonSync(test.json, test.password);
                                assert_1.default.equal(wallet2.privateKey, test.privateKey, 'generated correct private key - ' + wallet2.privateKey);
                            }
                            if (test.mnemonic) {
    ...
    ['one', 'two', 'three'].forEach(function (i) {
        ...
        var wallet = corebc_1.corebc.Wallet.createRandom("cc", { path: "m/56'/82", extraEntropy: utils.randomHexString('test-' + i, 32) });
        it('encrypts and decrypts a random wallet - ' + i, function () {
            this.timeout(1200000);
            return wallet.encrypt(password).then(function (json) {
                return corebc_1.corebc.Wallet.fromEncryptedJson(json, password).then(function (decryptedWallet) {
...
describe('Test Transaction Signing and Parsing', function () {
    function checkTransaction(parsedTransaction, test) {
        ...
        ['nonce', 'energyLimit', 'energyPrice', 'to', 'value', 'data'].forEach(function (key) {
            ...
            if (["energyLimit", "energyPrice", "value"].indexOf(key) >= 0) {
                assert_1.default.ok((corebc_1.corebc.BigNumber.isBigNumber(value)), 'parsed into a big number - ' + key);
                ...
                value = corebc_1.corebc.utils.hexlify(value);
                if (!expected || expected === '0x') {
                    expected = '0x00';
                }
            }
                ...
                if (value) {
                    ...
                    corebc_1.corebc.utils.getAddress(value);
    ...
    var tests = (0, corebc_testcases_1.loadTests)('transactions');
    tests.forEach(function (test) {
        it(('parses and signs transaction - ' + test.name), function () {
            this.timeout(120000);
            var signingKey = new corebc_1.corebc.utils.SigningKey(test.privateKey);
            var signDigest = signingKey.signDigest.bind(signingKey);
            // Legacy parsing unsigned transaction
            checkTransaction(corebc_1.corebc.utils.parseTransaction(test.unsignedTransaction), test);
            var parsedTransaction = corebc_1.corebc.utils.parseTransaction(test.signedTransaction);
            var transaction = checkTransaction(parsedTransaction, test);
            // Legacy signed transaction ecrecover
            assert_1.default.equal(parsedTransaction.from, corebc_1.corebc.utils.getAddress(test.accountAddress), 'computed from');
            ...
            (function () {
                var unsignedTx = corebc_1.corebc.utils.serializeTransaction(transaction);
                ...
                var signature = signDigest(corebc_1.corebc.utils.sha256(unsignedTx));
                assert_1.default.equal(corebc_1.corebc.utils.serializeTransaction(transaction, signature), test.signedTransaction, 'signs transaction (legacy)');
            })();
            // EIP155
            // EIP-155 parsing unsigned transaction
            var parsedUnsignedTransactionNetworkId5 = corebc_1.corebc.utils.parseTransaction(test.unsignedTransactionNetworkId5);
            ...
            // EIP-155 fields
            var parsedTransactionNetworkId5 = corebc_1.corebc.utils.parseTransaction(test.signedTransactionNetworkId5);
            ...
            (function () {
                ...
                var signature = signDigest(corebc_1.corebc.utils.sha256(unsignedTx));
                assert_1.default.equal(corebc_1.corebc.utils.serializeTransaction(transaction, signature), test.signedTransactionNetworkId5, 'signs transaction (eip155)');
            })();
        });
    });
    tests.forEach(function (test) {
...
describe('Test Signing Messages', function () {
    var tests = [
        ...
        // See: https://github.com/ethers-io/ethers.js/issues/80
        {
            ...
            message: corebc_1.corebc.utils.arrayify('0x47173285a8d7341e5e972fc677286384f802f8ef42a5ec5f03bbfa254cb01fad'),
        ...
        // See: https://github.com/ethers-io/ethers.js/issues/85
        {
            ...
            message: corebc_1.corebc.utils.arrayify(corebc_1.corebc.utils.id('0x7f23b5eed5bc7e89f267f339561b2697faab234a2')),
    ...
    tests.forEach(function (test) {
        it(('verifies a message "' + test.name + '"'), function () {
            this.timeout(120000);
            var address = corebc_1.corebc.utils.verifyMessage(test.message, test.signature, test.prefix);
    ...
    tests.forEach(function (test) {
        it(('hashes a message "' + test.name + '"'), function () {
            this.timeout(120000);
            var hash = corebc_1.corebc.utils.hashMessage(test.message);
...
describe("Serialize Transactions", function () {
    it("allows odd-length numeric values", function () {
        corebc_1.corebc.utils.serializeTransaction({
            energyLimit: "0x1",
            energyPrice: "0x1",
            value: "0x1"
        });
...
describe("Wallet Errors", function () {
    it("fails on privateKey/address mismatch", function () {
        assert_1.default.throws(function () {
            var wallet = new corebc_1.corebc.Wallet({
    ...
    it("fails on from mismatch", function () {
        var _this = this;
        var wallet = new corebc_1.corebc.Wallet("0x6a73cd9b03647e83ef937888a5258a26e4c766dbf41ddd974f15e32d09cfe9c0", "cc");
        return new Promise(function (resolve, reject) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 2:
                        error_1 = _a.sent();
                        if (error_1.code === corebc_1.corebc.utils.Logger.errors.INVALID_ARGUMENT && error_1.argument === "transaction.from") {
                            resolve(true);
                            return [2 /*return*/];
                        }
                        return [3 /*break*/, 3];
