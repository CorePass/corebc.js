...
function randomCase(seed, text) {
    return text.split("").map(function (c, index) {
        if ((0, corebc_testcases_1.randomNumber)(seed + "-" + index, 0, 2)) {
...
describe('Test HD Node Derivation is Case Agnostic', function () {
    var tests = (0, corebc_testcases_1.loadTests)('hdnode');
    tests.forEach(function (test) {
        ...
        it("Normalizes case - " + test.name, function () {
            this.timeout(10000);
            var wordlist = (corebc_1.corebc.wordlists)[test.locale];
            var rootNode = corebc_1.corebc.utils.HDNode.fromMnemonic(test.mnemonic, test.prefix, test.password || null, wordlist);
            var altMnemonic = randomCase(test.name, test.mnemonic);
            var altNode = corebc_1.corebc.utils.HDNode.fromMnemonic(altMnemonic, test.prefix, test.password || null, wordlist);
...
describe('Test HD Node Derivation from Seed', function () {
    ...
    tests.forEach(function (test) {
        ...
        it('Derives the HD nodes - ' + test.name, function () {
            this.timeout(10000);
            var rootNode = corebc_1.corebc.utils.HDNode.fromSeed(test.seed, test.prefix);
            test.hdnodes.forEach(function (nodeTest) {
                ...
                var wallet = new corebc_1.corebc.Wallet(node.privateKey, test.prefix);
...
describe('Test HD Node Derivation from Mnemonic', function () {
    ...
    tests.forEach(function (test) {
        ...
        it('Derives the HD nodes - ' + test.name, function () {
            this.timeout(10000);
            var rootNode = corebc_1.corebc.utils.HDNode.fromMnemonic(test.mnemonic, test.password || null);
            test.hdnodes.forEach(function (nodeTest) {
                var node = rootNode.derivePath(nodeTest.path);
...
describe('Test HD Mnemonic Phrases', function testMnemonic() {
    ...
    tests.forEach(function (test) {
        ...
        it(('converts mnemonic phrases - ' + test.name), function () {
            ...
            assert_1.default.equal(corebc_1.corebc.utils.mnemonicToSeed(test.mnemonic, test.password), test.seed, 'Converts mnemonic to seed - ' + test.mnemonic + ':' + test.password);
            // Test default english
            if (test.locale === "en") {
                assert_1.default.equal(corebc_1.corebc.utils.entropyToMnemonic(test.entropy), test.mnemonic, "Converts entropy to mnemonic " + test.name + " (default en)");
                assert_1.default.equal(corebc_1.corebc.utils.mnemonicToEntropy(test.mnemonic), test.entropy, "Converts mnemonic to entropy - " + test.mnemonic + " (default en)");
            }
            var wordlist = (corebc_1.corebc.wordlists)[test.locale];
            var mnemonic = corebc_1.corebc.utils.entropyToMnemonic(test.entropy, wordlist);
            assert_1.default.equal(mnemonic.normalize('NFKD'), test.mnemonic.normalize('NFKD'), 'Converts entropy to mnemonic ' + test.name);
            assert_1.default.equal(corebc_1.corebc.utils.mnemonicToEntropy(test.mnemonic, wordlist), test.entropy, 'Converts mnemonic to entropy - ' + test.mnemonic);
        });
    });
});
describe("HD error cases", function () {
    ...
    var root = corebc_1.corebc.utils.HDNode.fromSeed("0xdeadbeefdeadbeefdeadbeefdeadbeef", "cc");
    ...
    it("fails on invalid mnemonic length", function () {
        ...
        assert_1.default.ok(corebc_1.corebc.utils.isValidMnemonic(zeroMnemonicCS));
        assert_1.default.ok(!corebc_1.corebc.utils.isValidMnemonic(zeroMnemonic));
        assert_1.default.ok(!corebc_1.corebc.utils.isValidMnemonic(shortMnemonic));
        assert_1.default.throws(function () {
            corebc_1.corebc.utils.mnemonicToEntropy(shortMnemonic);
        }, function (error) {
            return true;
        });
    });
    it("fails on invalid checksum", function () {
        assert_1.default.throws(function () {
            corebc_1.corebc.utils.mnemonicToEntropy(zeroMnemonic);
        }, function (error) {
            return true;
        });
    });
    it("fails on unknown locale", function () {
        assert_1.default.throws(function () {
            corebc_1.corebc.utils.HDNode.fromMnemonic(zeroMnemonicCS, "foobar", "xx");
        }, function (error) {
            return true;
        });
    });
