...
function equals(actual, expected) {
    ...
    // Uint8Array
    if (expected.buffer) {
        if (!corebc_1.corebc.utils.isHexString(actual)) {
            return false;
        }
        actual = corebc_1.corebc.utils.arrayify(actual);
    ...
    // Maybe address?
    try {
        var actualAddress = corebc_1.corebc.utils.getAddress(actual);
        var expectedAddress = corebc_1.corebc.utils.getAddress(expected);
...
function getValues(object, named) {
    ...
    switch (object.type) {
        ...
        case 'string':
            return object.value;
        case 'buffer':
            return corebc_1.corebc.utils.arrayify(object.value);
        case 'tuple':
...
describe('ABI Coder Encoding', function () {
    var coder = corebc_1.corebc.utils.defaultAbiCoder;
    var tests = (0, corebc_testcases_1.loadTests)('contract-interface');
...
describe('ABI Coder ABIv2 Encoding', function () {
    var coder = corebc_1.corebc.utils.defaultAbiCoder;
    var tests = (0, corebc_testcases_1.loadTests)('contract-interface-abi2');
    tests.forEach(function (test) {
...
describe('Test Contract Events', function () {
    var tests = (0, corebc_testcases_1.loadTests)('contract-events');
    tests.forEach(function (test, index) {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function () {
            this.timeout(120000);
            var iface = new corebc_1.corebc.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function () {
            ...
            test.normalizedValues.forEach(function (expected, index) {
                if (test.indexed[index]) {
                    assert_1.default.ok((corebc_1.corebc.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                }
                else {
...
describe('Test Interface Signatures', function () {
    var tests = (0, corebc_testcases_1.loadTests)('contract-signatures');
    tests.forEach(function (test) {
        it('derives the correct signature - ' + test.name, function () {
            var iface = new corebc_1.corebc.utils.Interface(test.abi);
            this.timeout(120000);
    ...
    it('derives correct description for human-readable ABI', function () {
        var iface = new corebc_1.corebc.utils.Interface(["function transfer(address from, uint amount)"]);
        [
            "transfer",
    ...
    it('parses transaction function', function () {
        ...
        var tx = corebc_1.corebc.utils.parseTransaction(rawTx);
...
describe('Test Number Coder', function () {
    ...
    it('encodes zero', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: corebc_1.corebc.constants.Zero }
        ];
    ...
    it('encodes one', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: corebc_1.corebc.constants.One }
        ];
        var expected = oneHex;
        ['uint8', 'uint256', 'int8', 'int256'].forEach(function (type) {
            ZeroValues.forEach(function (value) {
    ...
    it('encodes negative one', function () {
        var Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: corebc_1.corebc.constants.NegativeOne }
        ];
        var expected = maxHex;
    ...
    it('encodes full uint8 range', function () {
        for (var i = 0; i < 256; i++) {
            ...
            expected += corebc_1.corebc.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function () {
        for (var i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += corebc_1.corebc.utils.hexlify(256 + i).substring(2);
            }
            else {
...
describe('Test Filters', function () {
    ...
    function doTest(test) {
        it(test.name, function () {
            var iface = new corebc_1.corebc.utils.Interface([test.signature]);
...
describe("Test ParamType Parser", function () {
    ...
    Tests.forEach(function (test) {
        it("allows correct modifiers " + JSON.stringify(test.type), function () {
            var paramType = corebc_1.corebc.utils.ParamType.from(test.type);
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        contract = new corebc_1.corebc.Contract(addr, [
                    ...
                    case 3:
                        error_1 = _a.sent();
                        assert_1.default.equal(error_1.code, corebc_1.corebc.utils.Logger.errors.CALL_EXCEPTION, "error.code");
...
describe("Additional test cases", function () {
    ...
    it("allows addresses without the 0x", function () {
        var iface = new corebc_1.corebc.utils.Interface([
