...
export function loadTests(tag: string): any {
    ...
    if (Cache[filename] == null) {
        ...
        try {
            ...
            const size = parseInt(comps[0]), compressedData = corebc.utils.base64.decode(comps[1]);
            const uncompressedData = new Uint8Array(size);
            inflate(compressedData, uncompressedData);
            Cache[filename] = JSON.parse(corebc.utils.toUtf8String(uncompressedData));
        } catch (error) {
...
export function loadData(filename: string): Uint8Array {
    ...
    return corebc.utils.base64.decode((<any>data)[filename]);
}
