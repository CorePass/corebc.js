...
function loadTests(tag) {
    ...
    if (Cache[filename] == null) {
        ...
        try {
            ...
            var size = parseInt(comps[0]), compressedData = corebc_1.corebc.utils.base64.decode(comps[1]);
            var uncompressedData = new Uint8Array(size);
            (0, tiny_inflate_1.default)(compressedData, uncompressedData);
            Cache[filename] = JSON.parse(corebc_1.corebc.utils.toUtf8String(uncompressedData));
        }
        catch (error) {
...
function loadData(filename) {
    ...
    return corebc_1.corebc.utils.base64.decode(browser_data_json_1.default[filename]);
}
