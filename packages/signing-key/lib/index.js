...
var SigningKey = /** @class */ (function () {
    function SigningKey(privateKey) {
        (0, corebc_properties_1.defineReadOnly)(this, "privateKey", (0, corebc_bytes_1.hexlify)(privateKey));
        (0, corebc_properties_1.defineReadOnly)(this, "publicKey", computePublicKey(privateKey));
        (0, corebc_properties_1.defineReadOnly)(this, "_isSigningKey", true);
    }
    SigningKey.prototype.signDigest = function (digest) {
        var pub = computePublicKey(this.privateKey);
        var sig = sign(this.privateKey, digest);
        return (0, corebc_bytes_1.hexConcat)([sig, pub]);
    };
...
function sign(key, digest) {
    var keyBuffer = Buffer.from((0, corebc_bytes_1.arrayify)(key));
    if (keyBuffer.length !== 57) {
        logger.throwArgumentError("invalid private key", "key", "[REDACTED]");
    }
    var digestBuffer = Buffer.from((0, corebc_bytes_1.arrayify)(digest));
    ...
    if (keyBuffer[56] > 127) {
        ...
        return (0, corebc_bytes_1.hexlify)(sig_1);
    }
    var sig = ed448_1.default.sign(digestBuffer, keyBuffer);
    return (0, corebc_bytes_1.hexlify)(sig);
}
function recoverPublicKey(digest, signature) {
    ...
    var sigBuffer = Buffer.from((0, corebc_bytes_1.arrayify)(signature));
    ...
    if (ed448_1.default.verify(digestBuffer, sig, pub)) {
        return (0, corebc_bytes_1.hexlify)(pub);
    }
    logger.throwArgumentError("invalid signature", "signature", signature);
    return "";
}
...
function computePublicKey(key) {
    var bytes = Buffer.from((0, corebc_bytes_1.arrayify)(key));
    if (bytes.length !== 57) {
        logger.throwArgumentError("invalid private key", "key", "[REDACTED]");
    }
    if (bytes[56] > 127) {
        ...
        scalar[0] &= 0xfc;
        scalar[55] |= 0x80;
        var pub_1 = ed448_1.default.publicKeyFromScalar(scalar);
        return (0, corebc_bytes_1.hexlify)(pub_1);
    }
