...
function _encode(object) {
    ...
    if (!(0, data_js_1.isBytesLike)(object)) {
        logger.throwArgumentError("RLP object must be BytesLike", "object", object);
    }
    const data = Array.prototype.slice.call((0, data_js_1.arrayify)(object));
...
function encode(object) {
    return (0, data_js_1.hexlify)(_encode(object));
}
...
function _decode(data, offset) {
    ...
    if (data[offset] >= 0xf8) {
        ...
        if (offset + 1 + lengthLength > data.length) {
            logger.throwError("data short segment too short", logger_js_1.Logger.errors.BUFFER_OVERRUN, {});
        }
        const length = unarrayifyInteger(data, offset + 1, lengthLength);
        if (offset + 1 + lengthLength + length > data.length) {
            logger.throwError("data long segment too short", logger_js_1.Logger.errors.BUFFER_OVERRUN, {});
        }
    ...
    return { consumed: 1, result: (0, data_js_1.hexlify)(data[offset]) };
}
function decode(data) {
    const bytes = (0, data_js_1.arrayify)(data);
